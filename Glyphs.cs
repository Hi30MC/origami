        ...
        QApi.RunDuringCycle(static (sim, part, pss, first) => {
            ...
            if (type == Translation)
            {
                if (first)
                {
                    ...
                    if (!sim.FindAtom(bowl).method_99(out AtomReference subject))
                    {
                        return;
                    }
                    ...
                    Brimstone.API.ChangeAtom(subject, output);
                    subject.field_2279.field_2276 = new class_168(seb, 0, (enum_132) 1, subject.field_2280, class_238.field_1989.field_81.field_614, 30f);
                }
            }
        });
    }
